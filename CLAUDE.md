...
- Constitution Gate: CBF-QP 하드게이트 → LLM 소프트게이트 순서
...
- [ ] V3 Life Guard 노드 → agent_graph.py 연결 (방부장 승인 후)
- [ ] V3 활성화 시 `AgentState`에 `audit_trail`, `disappointment_score`, `decay_multiplier`, `is_suspended` 필드 추가 필요
- [ ] sync_api 인메모리 → Turso 교체
- [ ] POPEYEs: inface_connector, turso_reader, excel_generator 미구현
- [ ] 차후 종분화 단계에서 `transplant.py`에 자동 리네이밍 로직 추가 (이식체 → `Jiim-{site_name}`)
