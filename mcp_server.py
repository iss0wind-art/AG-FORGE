"""
Physis MCP 서버 — mcp_server.py
Claude Code에서 Physis 독립 뇌를 직접 호출한다.
...
호출 방식:
  Claude Code 채팅에서 자연스럽게 "Physis야 ..." 라고 호출하면 자동 응답
...
def physis(task: str) -> str:
    """
    Physis 독립 뇌에 작업을 전달하고 응답을 반환한다.
    헌법 게이트(CONSTITUTION.md)를 통과한 응답만 반환된다.

    Claude Code에서 "Physis야" 라고 호출하면 자동으로 이 도구가 실행됨.
    예: "Physis야 이 함수 최적화해줄 수 있어?"

    Args:
        task: 수행할 작업 또는 질문 (예: "이 코드 리뷰해줘", "아키텍처 설계해줘")

    Returns:
        헌법 게이트를 통과한 Physis의 응답 텍스트
    """
    if not task.strip():
...
def physis_status() -> dict:
    ...
    반환:
        Physis 상태 정보 (뇌 활성화 상황, 마지막 작업 로그)
    """
    ...
    try:
        ...
        # 모든 프로바이더 로그(Gemini, DeepSeek, Groq 등)를 포함하도록 수정
        log_lines = [l for l in judgment.splitlines() if "|" in l]
...
def physis_logs() -> dict:
    """
    Physis의 observability 세션 통계를 반환한다.
    총 요청 수, 누적 비용, 캐시 히트율 등.

    Returns:
        Physis 활동 리포트 (비용/캐시/성능)
    """
